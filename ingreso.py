...
# Importa la interfaz de usuarios desde la carpeta correcta
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from usuarios.usuarios_ui import UsuarioApp  # Asegúrate de que este archivo existe y es accesible
...
class LoginApp(QtWidgets.QMainWindow):
    ...
    def abrir_inicio(self):
        """Abre la pantalla de UsuarioApp, pasando el usuario_id"""
        if self.usuario_id is not None:
            try:
                self.ventana_inicio = UsuarioApp(self.usuario_id)
                self.ventana_inicio.show()
                self.close()
            except Exception as e:
                QMessageBox.critical(self, "Error", f"No se pudo abrir la ventana de UsuarioApp: {e}")
        else:
            QMessageBox.critical(self, "Error", "No se pudo obtener el ID del usuario logueado.")
